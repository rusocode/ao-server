<!-- Generated: 2026-05-08 | Files scanned: 258 classes / 47 packages | Token estimate: ~350 -->
...
| `ch.qos.logback:logback-classic` | 1.5.18 | SLF4J logging impl |
...
None — the server is fully self-contained. All game data is in local files under `data/`. No external APIs, databases, message brokers, or cloud services.
...
```
`server-security` provides `SecurityManager` implementations injected via `SecurityModule` at runtime using the class name from `project.properties`.
